...
Use `squeue` to see your work in Slurm and `scancel` to remove it. The output will appear in *output* and *error* files
with Slurm job number. Although running the scripts takes up to only ten minutes (each),
