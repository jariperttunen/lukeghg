...
if __name__ == "__main__":
    ...
    args_group.add_argument('--format_only',action="store_true",help="Genrate Table 6.4-1 as is")
    args_group.add_argument('--check_total',action="store_true",help="Calculate self and compare totals")
    ...
    if args.format_only:
        ...
        columnls=['Mineral','Undrained','Herb-rich type','Vaccinium myrtillus type','Vaccinium vitis-idaea type','Dwarf shrub type','Cladina type']
        df = pd.read_table(args.f1,sep=',',usecols=columnls)
        df['Drained organic'] = df['Herb-rich type']+df['Vaccinium myrtillus type']+df['Vaccinium vitis-idaea type'] +df['Dwarf shrub type']+df['Cladina type']
        df['Total organic']=df['Drained organic']+df['Undrained']
        df['Total (Mineral+Organic)']=df['Mineral']+df['Total organic']
